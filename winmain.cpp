...
*/
const WCHAR CLASS_NAME[]  = L"SPIVIDEOPLAY";
const WCHAR WINDOW_NAME[] = L"spivideoplay";
...
#include <propvarutil.h> //for IMFPMediaItem::GetDuration()
...
void CALLBACK StartGlobalProcess(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	while( ((nowstamp_ms-global_startstamp_ms)/1000.0f)<global_duration_sec || (mediaduration_s<0.0f) || ((global_duration_sec<0.0f)&&(mediaduration_s>0.0f)&&(((nowstamp_ms-global_startstamp_ms)/1000.0f)<mediaduration_s)) )
	{
		...
		//Sleep(3000); //long enough so media is opened
		if(bcalled==false)
		//if(1)
		{
			if(g_pPlayer)
			{
				bcalled=true;
				HRESULT hr = g_pPlayer->GetDuration(MFP_POSITIONTYPE_100NS, &myPROPVARIANT);
